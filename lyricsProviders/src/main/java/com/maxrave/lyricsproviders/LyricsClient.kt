...
import com.maxrave.lyricsproviders.parser.MusixmatchLyricsParser.parseMusixmatchLyrics
import com.maxrave.lyricsproviders.parser.MusixmatchLyricsParser.parseUnsyncedLyrics
...
class LyricsClient(
    ...
    suspend fun postMusixmatchCredentials(
        ...
        // Primero, obtener la respuesta completa como string para diagnosticar
        println("Raw Musixmatch response: ${request.bodyAsText()}")
        
        // Intentar deserializar el cuerpo
        val response = request.body<MusixmatchCredential>()
        
        // Verificar explícitamente el código de estado en el header
        if (response.message.header.status_code != 200) {
            println("Musixmatch authentication failed with status code: ${response.message.header.status_code}")
            return@runCatching request  // Retornar la solicitud original para manejar el error de autenticación
        }
        
        // Verifica si la respuesta tiene body y si contiene los datos necesarios
        if (response.message.body.isNotEmpty() && 
            response.message.body.firstOrNull()?.credential?.error == null &&
            response.message.body.firstOrNull()?.credential?.account != null
        ) {
